...
struct ChessBoard{
    ...
    Move negamax(int depth, bool minimize , int alpha, int beta){
        Move best_move;
        									
        if(0 == depth){
            best_move.score = sign[minimize]*score();
            return best_move;
        }
        ...
        for(auto & from : moverPieces()){
            for(auto & to : possibleMoves(from.first)){
                ...
                Move option = branch.negamax(depth-1, !minimize , -beta, -alpha);
				option.score = -option.score;
                if((option.score > best_move.score)){
                    best_move.score = option.score;
                    best_move.from = from.first;
                    best_move.to = to;
                }
				if (option.score >alpha) alpha = option.score;
				if (alpha >= beta) break;
            }
        }
        return best_move;
    ...
    void AIMove(){
        ...
        Move m = negamax(4,minimize , -1000000 , 1000000);
